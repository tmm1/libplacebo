                            ...
                            const struct pl_render_params *params)
{
    ...
    for (int i = 0; i < image->num_planes; i++) {
        ...
        // TODO: do we need to take into account the sample order here??
        for (int c = 0; c < PL_ARRAY_SIZE(grain_params.channels); c++)
            grain_params.channels[c] = plane->component_mapping[c];
