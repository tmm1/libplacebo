                         ...
                         const struct pl_grain_params *params)
{
    ...
    if (!sh_try_compute(sh, bw, bh, false, 0)) {
        PL_ERR(sh, "AV1 film grain shader requires compute shaders!");
        return;
    }
    ...
    // TODO: Try and replace these by fragment-shader friendly versions?
    GLSLH("#define block_id gl_WorkGroupID       \n"
          "#define local_id gl_LocalInvocationID \n"
